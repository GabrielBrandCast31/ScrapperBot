...
export const Route = createFileRoute("/importar")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Importe um .txt exportado do WhatsApp criando um cliente novo." },
    ],
  }),
    ...
    <div className="p-6 space-y-5 max-w-2xl mx-auto">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Crie um cliente novo a partir de um .txt exportado do WhatsApp. Para
          adicionar mensagens a uma conversa <strong>já existente</strong>, use o
          botão "Sincronizar" na linha do cliente em <Link to="/conversas" className="underline">Conversas</Link>.
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              type="submit"
              disabled={!file || !nome.trim() || importar.isPending}
            ...
            <div className="mt-5 flex items-start gap-3 rounded-lg border border-primary/30 bg-primary/10 p-4">
              ...
              <div className="flex-1 text-sm">
                <p className="font-medium">
                  {importar.data.chat_name} — {importar.data.novas} mensagem(ns)
                  importada(s).
                </p>
                <p className="text-xs text-muted-foreground">
                  {importar.data.total_linhas} linhas processadas,{" "}
                  {importar.data.puladas_sistema} sistema,{" "}
                  {importar.data.puladas_vazias} vazias. Duplicadas foram ignoradas.
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
              ...
              <Button asChild size="sm" variant="outline">
                <Link to="/conversas/$id" params={{ id: importar.data.chat_id }}>
                  Ver
                </Link>
              </Button>
            </div>
