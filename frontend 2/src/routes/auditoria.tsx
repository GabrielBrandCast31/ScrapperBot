...
import { Sparkles, Eye, Loader2, ShieldCheck } from "lucide-react";
...
export const Route = createFileRoute("/auditoria")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Resumos automáticos das conversas ativas, gerados pela IA." },
    ],
  }),
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        ...
        <Button size="sm" className="gap-2" onClick={() => rodar.mutate()} disabled={rodar.isPending}>
        ...
        {sumarios.map((s) => (
          <Card key={s.id} className="flex h-full flex-col">
            <CardHeader className="pb-2">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <CardTitle className="text-sm line-clamp-2">{s.chat_name || s.chat_id}</CardTitle>
                  <CardDescription className="text-xs">
                    {s.quando} · {s.qtd_msgs} msg analisadas
                  </CardDescription>
                </div>
                <span className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-semibold text-primary">IA</span>
              </div>
            </CardHeader>
            <CardContent className="flex-1 space-y-3">
              ...
              {s.janela && (
                <p className="text-[11px] text-muted-foreground border-t border-border/60 pt-2">Janela: {s.janela}</p>
              )}
      ...
      <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-xs text-muted-foreground">
        <strong className="text-foreground">Como funciona:</strong> a auditoria varre as conversas que receberam mensagem nas últimas 1h (ou 3h à noite, pra economizar tokens), monta um transcript de cada uma e pede pro <code>gpt-4o-mini</code> um resumo de 3-5 linhas (cliente pediu / equipe respondeu / o que ficou aberto).
      </div>
    </div>
  );
}
